...
from gesda_knowledge_base_embeddings import LocalEmbeddings
...
DEFAULT_INDEX_PATH = "/mnt/nlp/scratch/home/belghmi/indexes/gesda_index"
LLM_MODEL_NAME = "swiss-ai/Apertus-8B-Instruct-2509"


SYSTEM_PROMPT = """You are a precise research assistant specializing in science and technology foresight, grounded in the GESDA Science Breakthrough Radar reports.
...
Always cite your sources by referencing the radar edition and page number (e.g. "gesda_radar_2023, Page 47").
...
def retrieve_documents(vector_db: FAISS, query: str, k: int = 5) -> list[str]:
    retrieved_docs = vector_db.similarity_search(query=query, k=k)
    return [_format_doc(doc) for doc in retrieved_docs]
...
class RAG:
    ...
    def batch_retrieve(self, queries: list[str]) -> list[list[str]]:
        """Embed all queries at once, then retrieve and format documents for each."""
        ...
        for emb in tqdm(embeddings, desc="Searching index"):
            ...
            all_docs.append([_format_doc(doc) for doc in docs])
        return all_docs
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run a RAG query against the GESDA FAISS index.")
    ...
    parser.add_argument("--embedding-model", type=str, default=DEFAULT_MODEL_PATH)
    parser.add_argument("--llm", type=str, default=LLM_MODEL_NAME)
